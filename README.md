...
`app/model.py` in this repo is a placeholder architecture with random weights
(`models/classifier/dummy.pth`) — it exists so the full pipeline runs
end-to-end for integration/infra work. It is **not** the production fatigue
classifier and its predictions are not meaningful. See `private/README.md`
for how to swap in the real model for a production deployment.
...
MODEL_PATH=models/classifier/dummy.pth
